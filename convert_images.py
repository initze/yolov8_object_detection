...
def process_files(indir: Path, mipps_dir: Path, chunksize: int, outdir: Path, mipps_bin: str, mipps_file: str):
...
@use_yaml_config()
def main(
    data_dir: Annotated[Path, typer.Option(help="Path to the data directory")] = Path(r"N:\Response\Restricted_Airborne\MACS\Alaska\2024_Perma-X_Alaska\01_raw_data"),
    out_dir_base: Annotated[Path, typer.Option(help="Path to the output directory")] = Path(r"S:\p_initze\yolov8_object_detection\data"),
    ...
    for project_name in projects[:]:
        ...
        try:
            if not outdir.exists():
                process_files(indir, Path(mipps_dir), chunksize, outdir, mipps_bin, mipps_file)
            else:
